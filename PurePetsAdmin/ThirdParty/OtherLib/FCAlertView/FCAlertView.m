...
- (id)init
{
    ...
    if (self) {
        ...
        _customImageScale = 1;
        _titleFont    = [Styling fontBold:15];     // Default title font
        _subtitleFont = [Styling fontMedium:16];   // Default subtitle font
...
- (void)drawRect:(CGRect)rect {
    ...
    if (_subtitleFont != nil)
        ...
        descriptionLabel.font = [Styling fontMedium:16];
    else
        descriptionLabel.font = [Styling fontMedium:16];
    ...
    if (alertTextFields.count > 0) {
        ...
        for (int i = 0; i < MIN(alertTextFields.count,4); i++) {
            ...
            UITextField *tf = [[UITextField alloc] initWithFrame:CGRectMake(12.5, descriptionLabel.frame.size.height + descriptionLabel.frame.origin.y + 10.5 + 45*i, alertViewFrame.size.width - 25, 44)];
            ...
            if (self.darkTheme)
                ...
                tf.backgroundColor = AppBackgroundClr;
            ...
            [tf setFont:[Styling fontMedium:16]];
    ...
    if (_numberOfButtons == 0) { // View only contains DONE/DISMISS Button
        ...
        doneButton.titleLabel.font = [Styling fontMedium:15];
        ...
        otherButton.titleLabel.font =  [Styling fontMedium:15];
        ...
        firstButton.titleLabel.font =  [Styling fontMedium:15];
        if (self.firstButtonCustomFont)
        ...
        secondButton.titleLabel.font =  [Styling fontMedium:16];
...
- (void)showAlertView {
    ...
    [UIView animateWithDuration:0.3 delay:0.0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        if (self->_bounceAnimations) {
            if (!self->_animateAlertInFromTop && !self->_animateAlertInFromLeft &&
                !self->_animateAlertInFromRight && !self->_animateAlertInFromBottom) {
                self->alertViewContents.transform = CGAffineTransformMakeScale(0.95, 0.95);
            }
            if (self->_animateAlertInFromTop)
                self->alertViewContents.frame = CGRectOffset(self->alertViewFrame, 0, 7.5);
            if (self->_animateAlertInFromRight)
                self->alertViewContents.frame = CGRectOffset(self->alertViewFrame, -7.5, 0);
            if (self->_animateAlertInFromBottom)
                self->alertViewContents.frame = CGRectOffset(self->alertViewFrame, 0, -7.5);
            if (self->_animateAlertInFromLeft)
                self->alertViewContents.frame = CGRectOffset(self->alertViewFrame, 7.5, 0);
        } else {
            self->alertViewContents.transform = CGAffineTransformIdentity;
            self->alertViewContents.frame = self->alertViewFrame;
        }
        ...
        if (self.autoHideSeconds != 0) {
            [self performSelector:@selector(dismissAlertView)
                       withObject:nil
                       afterDelay:self.autoHideSeconds];
        }
    }];
...
- (void)dismissAlertView {
    ...
    [UIView animateWithDuration:0.175 animations:^{
        ...
        if (!self->_animateAlertOutToTop && !self->_animateAlertOutToLeft &&
            !self->_animateAlertOutToRight && !self->_animateAlertOutToBottom) {
            self.alpha = 0;
            self->backgroundVisualEffectView.alpha = 0;
            self->alertViewContents.transform = CGAffineTransformMakeScale(0.9, 0.9);
        } else {
            // … same pattern, but use self-> for ivars
        }
    } completion:^(BOOL finished) {
        __strong typeof(weakSelf) self = weakSelf;
        if (!self) return;
...
- (void)handleButton:(id)sender {
    ...
    NSDictionary *btnDict = [alertButtons objectAtIndex:btn.tag];
    ...
    if ([strongDelegate respondsToSelector:@selector(FCAlertView:clickedButtonIndex:buttonTitle:)]) {
        UIButton *btn = (UIButton *)sender;
        [strongDelegate FCAlertView:self clickedButtonIndex:btn.tag buttonTitle:clickedButton.titleLabel.text];
    }
...
- (void)pp_bounceView:(UIView *)view {
    if (!_bounceAnimations) return;
    ...
    [UIView animateWithDuration:0.2
        ...
        [UIView animateWithDuration:0.35
                            ...
                            options:UIViewAnimationOptionCurveEaseInOut
                         animations:^{
            strongView.transform = CGAffineTransformIdentity;
        } completion:nil];
    }];
}


- (void)pp_handleRating:(NSInteger)rating item:(UIView *)item {
    if (currentRating == rating) {
        currentRating = 0; // deselect if same
    } else {
        currentRating = rating;
    }
    [self setActiveRating:currentRating];
    
    [self pp_bounceView:item];
}

- (void)rate1Triggered { [self pp_handleRating:1 item:item1]; }
- (void)rate2Triggered { [self pp_handleRating:2 item:item2]; }
- (void)rate3Triggered { [self pp_handleRating:3 item:item3]; }
- (void)rate4Triggered { [self pp_handleRating:4 item:item4]; }
- (void)rate5Triggered { [self pp_handleRating:5 item:item5]; }
