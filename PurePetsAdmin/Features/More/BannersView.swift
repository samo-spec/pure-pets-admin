...
struct AdminBannersView: View {
    ...
    private var dossierHeaderView: some View {
        VStack(alignment: .leading, spacing: AdminSpacing.xs) {
            ...
            Text(Language.get("CommandCenter_Tab_More", alter: "المزيد") + " / " + Language.get("Banners_Title", alter: "الإعلانات الشريطية"))
                .font(AdminType.caption1)
                .foregroundColor(AdminSurface.secondaryText)
                .padding(.top, 2)

            Text(Language.get("Banners_Title", alter: "إدارة البنرات الإعلانية"))
