...
struct AdminNotificationComposerView: View {
    ...
    private var dossierHeaderView: some View {
        VStack(alignment: .leading, spacing: AdminSpacing.xs) {
            ...
            Text(Language.get("CommandCenter_Tab_More", alter: "المزيد") + " / " + Language.get("NotificationComposer_Title", alter: "إرسال إشعار"))
                .font(AdminType.caption1)
                .foregroundColor(AdminSurface.secondaryText)
                .padding(.top, 2)

            Text(Language.get("NotificationComposer_Title", alter: "إنشاء وإرسال الإشعارات"))
